...
class TestRepo(val repoPath: String) {
    ...
    fun commit(message: String,
               author: Author = Author(email = Email(userEmail), name = userName),
               date: Date = Date(),
               timeZone: TimeZone = TimeZone.getDefault()): RevCommit {
        val personIdent = PersonIdent(author.name, author.email.value(), date, timeZone)
        return git.commit().setMessage(message).setAll(true)
                  .setAuthor(personIdent).setCommitter(personIdent).call()
    }

    fun destroy() {
        Runtime.getRuntime().exec("rm -rf $repoPath").waitFor()
    }
}
